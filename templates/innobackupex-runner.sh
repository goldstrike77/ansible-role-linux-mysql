...
$RCLONE --no-check-certificate --verbose --config="/etc/rclone/mysql.conf" mkdir mysql:{{ ansible_hostname | lower }} >> $TMPFILE
$RCLONE --no-check-certificate --bwlimit="{{ mysql_backupset_arg.cloud_bwlimit | default('10M') }}" --verbose --config="/etc/rclone/mysql.conf" {{ mysql_backupset_arg.cloud_event | default('sync') }} {{ mysql_path }}/backup/mysql mysql:{{ ansible_hostname | lower }}/mysql >> $TMPFILE
{% endif %}
